...
function NavItem({ content, setSelected, selected}) {
  ...
  return <div className={selected === content ? styles.active : ''} onClick={onSelect}>{content}</div>;
}
...
function SortByOptions(props){
  ...
  return (
    <>
        {arr.map(option => <NavItem key={option} content={option} {...props} />)}
    </>
  )
}

export default function ShopNav({setSelectedCategory, selectedCategory, sortBy, setSortBy}) {
  return (
    <nav className={styles.navContainer}>
      <div className={styles.navWrapper}>
        <ShopNavSection header="Categories">
          <Categories setSelected={setSelectedCategory} selected={selectedCategory} />
        </ShopNavSection>
